...
def _initialize():
	for key, value in dictAttrToVal.items():
		dictValToAttr.update({value: key})
	for key, value in dictMsgTypeToVal.items():
		dictValToMsgType.update({value: key})
...
def stun_test(sock, host, port, source_ip, source_port, send_data=""):
	...
	while not recvCorr:
		...
		if isinstance(msgtype, bytes):
			msgtype = msgtype.decode()
		bind_resp_msg = dictValToMsgType[msgtype] == "BindResponseMsg"
		recvTrainId = binascii.b2a_hex(buf[4:20]).upper()
		if isinstance(recvTrainId, bytes):
			recvTrainId = recvTrainId.decode()
		tranid_match = tranid.upper() == recvTrainId
